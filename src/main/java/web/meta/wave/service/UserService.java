...
public class UserService {
    ...
    public UserService(UserRepository userRepository, TokenService tokenService, NetworkService networkService, BalanceService balanceService, WalletService walletService) {
    ...
    public void createStartEthAccount(CustomUser user) {
        Long countNetworks = networkService.countAll();
        MetaToken token = walletService.getEthToken();
        for (long i = 1L; i <= countNetworks; i++) {
            networkService.findById(i).ifPresent(network -> {
                Optional<Balance> balance = balanceService.getBalanceByNetworkAndMetaTokenAndCustomUser(network, token, user);
                balance.ifPresentOrElse(balanceEth -> {
                            balanceService.updateBalance(network, token, user, BigDecimal.valueOf(1000));
                        },
                        () -> balanceService.addBalance(network, token, user, BigDecimal.valueOf(1000))
                );
            });
        }
    }
