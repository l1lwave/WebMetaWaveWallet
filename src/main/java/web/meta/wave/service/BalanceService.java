...
public class BalanceService {
    ...
    public List<Balance> getBalanceByNetworkAndCustomUser(Network network, CustomUser user) {
        return balanceRepository.findAllByNetworkAndCustomUser(network, user).stream()
                .filter(balance -> balance.getMetaToken() != null)
                .filter(balance -> balance.getMetaToken().getTokenCode() == null || balance.getMetaToken().getTokenCode() > 0)
                .toList();
    }
